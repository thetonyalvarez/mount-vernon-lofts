...
import type { BrochureFormData, EnhancedBrochureFormData } from "@/app/types"
...
export function BrochureForm() {
  ...
  useEffect(() => {
    ...
    setSubmissionId(id)
    metadataCollector.startFormTracking("brochure_form")
  }, [uniqueId])

  const handleFormChange = (field: keyof BrochureFormData, value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const metadata = metadataCollector.getMetadata("brochure_form", "brochure_page")

      const enhancedFormData: EnhancedBrochureFormData = {
        ...formData,
        modalId: "brochure_form",
        triggerSource: "brochure_page",
        submissionId
      }
      ...
      const payload = {
        ...
        metadata: {
          modalId: "brochure_form",
          modalTriggerSource: "brochure_page",
          siteUrl: window.location.origin,
          submissionId,
          sessionData: metadata.sessionData,
          deviceInfo: metadata.deviceInfo,
          interactionMetrics: metadata.interactionMetrics,
          leadScore: metadata.leadScore + 25 // Brochure interest adds 25 points
        }
      }
      ...
      // Track successful brochure request (reusing floor plans tracking for now)
      trackFloorPlansRequest(
        formData.brochureInterest,
        'brochure_page',
        true,
        {
  ...
  return (
    <section id="brochure-form" className="w-full bg-mvl-warm-white py-20 md:py-32">
            ...
            <div className="text-center mb-12">
              ...
              <StaggerItem>
                <p className="text-mvl-espresso text-lg md:text-xl font-light">
                  Get instant access to our comprehensive property brochure
                </p>
              </StaggerItem>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-xl p-8 md:p-12">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <ScrollReveal variant={fadeInUp} delay={0.3}>
                  <div className="grid md:grid-cols-2 gap-4">
                    <Select
                      value={formData.brochureInterest ?? ""}
                      onChange={(e) => handleFormChange("brochureInterest", e.target.value)}
                      className="border-gray-300 focus:border-mvl-coral focus:ring-mvl-coral transition-all duration-300"
                      disabled={isSubmitting}
                      required
                    >
                      <option value="">Information Interest</option>
                      <option value="all">Complete Information Package</option>
                ...
                <ScrollReveal variant={fadeInUp} delay={0.4}>
                  <Textarea
                    placeholder="Message (Optional) - Tell us about your ideal residence or any specific questions"
